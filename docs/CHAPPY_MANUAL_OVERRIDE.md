...
## 発走前チェック（2026-09-22 のレビューで追加）

運用ルールだけでなく、**コード側で機械的に検査する**。
`logic/chappy.load_manual_override` は次のいずれかに当てはまると採用せず、
自動Chappyの案に落とす（例外にはせず警告を残す）。

- `created_at` が無い
- `created_at` がISO8601として読めない
- `created_at` が**発走時刻以降**
- 発走時刻そのものが判らない（判定できないので保守側に倒す）

採用したカードには決定ログに `verified_pre_race: true` が残る。
つまり「結果を見たあとに置いたファイル」は採点に入らない。
